...
- Inline-кнопки для выбора способа оплаты, валюты RUB/USD, криптовалюты, категории и подтверждения
- Запрос номера кошелька для крипто-расходов
- Inline-кнопки для статуса операции: `Оплачен`, `На рассмотрении`, `Отказ`
- Хранение расходов в листе `Operations`
